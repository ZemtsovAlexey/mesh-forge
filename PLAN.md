...
- **Сервер (DESKTOP-HOME):** LM Studio, ComfyUI, MeshForge
...
|-------|------|------------|
| 1. Фото | JPG/PNG | ComfyUI Hunyuan MV → mesh QC |
| 2. Скан | STL/OBJ | PyMeshLab / Poisson |
| 3. Текст | промпт | ComfyUI text→views→mesh |

## Редактирование

- Text edit: LLM planner → JSON ops → trimesh
...
- [ ] PyMeshLab pipeline
- [ ] STL post-processing without Blender
...
| Скрипт | Назначение |
|--------|------------|
| `setup.ps1` | `uv sync` + ComfyUI + checkpoints |
| `setup-comfyui.ps1` | только ComfyUI |
| `start-comfyui.ps1` / `stop-comfyui.ps1` | управление ComfyUI |
| `run.ps1` | FastAPI через `uv run` (`-WithComfyUI` опционально) |
